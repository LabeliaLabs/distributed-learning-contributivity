...
def Support(the_scenario, sv_accuracy=0.01, alpha=0.95):
    """Return the vector of approximated Shapley values using the stratified monte-carlo method."""
    ...
    def not_twice_characteristic(permut):
        # Sort permut
        permut = np.sort(permut)
        if tuple(permut) in char_value_dict: # Return the characteristic_func(permut) if it was already computed
            return char_value_dict[tuple(permut)]
        else:  # Characteristic_func(permut) has not been computed yet, so we compute, store, and return characteristic_func(permut)
            small_node_list = np.array([preprocessed_node_list[i] for i in permut])
            char_value_dict[tuple(permut)] = fl_training.compute_test_score(
                small_node_list,
                the_scenario.epoch_count,
        ...
        while (
            ...
            for k in range(N):
                ...
                for  v in contributions[k][strata].values():
                    sigma[k, strata] +=  v[1]*(v[0]-mu[k, strata])**2
                    count+=v[1]
                sigma[k, strata] /= count-1
                sigma[k, strata] = np.sqrt(sigma[k, strata]) 
            shap = np.mean(mu, axis=0)
            var = np.zeros(N)  # variance of the
            for k in range(N):
                for strata in range(N):
